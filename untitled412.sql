...
UNIQUE_CITY AS (
SELECT CITY, MAX(LOCATION_ID) AS MAX_LOCATION_ID
FROM ALL_LOCATIONS
GROUP BY CITY
)
SELECT ALL_LOCATIONS.LOCATION_ID, POSTAL_CODE,ALL_LOCATIONS.CITY, COUNTRY_ID
FROM ALL_LOCATIONS JOIN UNIQUE_CITY
...
--Find all the employee whose salary are not in job range 

SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, EMPLOYEES.JOB_ID, SALARY, JOB_TITLE, MIN_SALARY, MAX_SALARY
